...
impl CompanyRepository for MyCompanyRepository {
    async fn find_by_id_vec(&self, id_vec: Vec<i64>) -> Result<Vec<Company>, DomainError> {
        let id_vec: Vec<i64> = id_vec.to_vec();
...
impl InternalCompanyRepository {
    async fn find_by_id_vec(
        ...
        let rows = sqlx::query_as!(
            CompanyRow,
            "SELECT * FROM companies WHERE company_cd IN (SELECT unnest($1::bigint[]))",
            &id_vec
        )
        .fetch_all(conn)
        .await?;
