...
pub async fn get_station_by_id(
    ...
    let line_service = LineService::new(ctx.line_repository(), ctx.station_repository());
    ...
    let lines = line_service
        ...
        .await?;
    let same_group_stations = station_service.get_by_group_id(data.station_g_cd).await?;
    ...
    let lines_response: Vec<LineResponse> = lines
        ...
        .map(|line| {
            ...
            let line_symbols = line_service.get_line_symbols(line.clone());
            line_response.line_symbols = line_symbols;

            let line_station = same_group_stations
                .iter()
                .find(|station| station.line_cd == line.clone().line_cd)
                .unwrap();
            line_response.station = Some(Box::new(line_station.clone().into()));

            line_response
        })
        .collect();
    let station_line = line_service.find_by_station_id(data.station_cd).await?;
    ...
    let line_symbols = line_service.get_line_symbols(station_line.clone());
    line_response.line_symbols = line_symbols;
...
pub async fn get_stations_by_line_id(
    ...
    let stations_lines = &line_service.get_stations_lines(stations).await?;
    let response_stations: &Vec<SingleStationResponse> = &line_service
        .get_response_stations_from_stations(stations, stations_lines)
        .await?;
