...
//! マクロで同じ定義から 2 つの型を作る。
//!
//! 値は `stationapi::model` の型から変換する。IPA や TTS セグメントは
...
impl From<model::TtsSegment> for TtsSegment {
    fn from(v: model::TtsSegment) -> Self {
        Self {
...
impl From<model::StationNumber> for StationNumber {
    fn from(v: model::StationNumber) -> Self {
        Self {
...
impl From<model::LineSymbol> for LineSymbol {
    fn from(v: model::LineSymbol) -> Self {
        Self {
...
impl From<model::Company> for Company {
    fn from(v: model::Company) -> Self {
        Self {
            id: Some(v.id as i32),
...
macro_rules! define_station {
    ($ident:ident, $name:literal) => {
        ...
        impl From<model::Station> for $ident {
            fn from(v: model::Station) -> Self {
                Self {
                    id: Some(v.id as i32),
...
macro_rules! define_line {
    ($ident:ident, $name:literal) => {
        ...
        impl From<model::Line> for $ident {
            fn from(v: model::Line) -> Self {
                Self {
                    id: Some(v.id as i32),
                    name_short: Some(v.name_short),
                    name_katakana: Some(v.name_katakana),
...
macro_rules! define_train_type {
    ($ident:ident, $name:literal) => {
        ...
        impl From<model::TrainType> for $ident {
            fn from(v: model::TrainType) -> Self {
                Self {
                    id: Some(v.id as i32),
...
impl From<model::TrainRouteSegment> for TrainRouteSegment {
    fn from(v: model::TrainRouteSegment) -> Self {
        Self {
