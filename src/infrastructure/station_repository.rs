        ...
        let station_row: Vec<StationRow> = sqlx::query_as(
            ...
            FROM
              ...
              LEFT OUTER JOIN `station_station_types` AS sst ON sst.line_group_cd = (
                ...
                -- LIMIT 1で最初のIDとなる値が求めているIDとは限らないのでソートしている
                ORDER BY
                  sst.id
                LIMIT
                  1
            ...
            WHERE
              s.station_cd = CASE WHEN sst.station_cd IS NOT NULL
              	THEN sst.station_cd
              	ELSE s.station_cd
              END
              AND l.line_cd = CASE WHEN sst.station_cd IS NULL THEN ? END
              AND s.line_cd = l.line_cd
        ORDER BY
          CASE WHEN sst.id IS NOT NULL THEN sst.id ELSE CONCAT(s.e_sort, s.station_cd) END",
        )
        .bind(line_id)
        .bind(station_id)
        .bind(station_id)
