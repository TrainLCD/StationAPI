...
`schema/public.graphql` の `Query` 型にあるルートフィールドを全て扱います。
...
速度の数字は条件次第でいくらでも動くので、以下を持たせています。

- **1 クエリにつき複数の引数を巡回します。** 同じ引数を投げ続けると、上流のメモ化や
  暖まった経路にだけ当たり続けて実態とずれます。地域と規模を散らした 3 組を回し、
  同じ反復では両環境に同じ引数を渡すので workload は完全に一致します。
- **フィールド選択を 2 段用意しています** (`--profile full|minimal`)。応答量が環境間で
  違うと、探索の速さではなく直列化と転送の量を測ってしまいます。`minimal` は ID 程度しか
  引かないので応答量の差に鈍く、2 つを比べれば差がどちら由来か切り分けられます。
- **`GET /__ping` をケースの区切りごとに挟みます。** エッジまでの往復を計測中ずっと追い、
  ネットワークの揺らぎと処理時間を切り分けます。
- **応答の食い違いを種類で分けます。** 件数・null の有無・型が違うもの (構造) は
  やっている仕事量が違うので速度を比較できません。並びや値だけの違いは仕事量を変えないので
  比較は成立します。レポートの集計は前者を除いて出します。

### 使い方

```bash
# 速度を測ってレポート (Markdown) まで出す。応答の一致も併せて調べる
python3 scripts/benchmark_graphql.py --iterations 30 --report-out bench.md

# 応答が一致するかだけ調べる (各引数 1 回ずつ)
python3 scripts/benchmark_graphql.py --parity

# 応答量の影響を落として探索の費用に寄せる
python3 scripts/benchmark_graphql.py --profile minimal --iterations 30

# 気になるクエリだけ (--list でクエリ名と引数の組を確認できる)
...
`--report-out` は計測条件・結果 (p50/p95/p99)・引数ごとの内訳・応答の一致・
外に出すときの注意までを 1 つの Markdown にまとめます。集計は
「応答が比較可能なクエリだけ」と「全クエリ (参考)」の 2 段で出るので、
公表には前者を使ってください。

### 計測上の注意

**公表する数字は利用者のいる地域から測ってください。** `GET /__ping` を併せて測るので
エッジまでの往復と処理時間は切り分けられますが、上流へ往復する構成が相手の場合は
計測地から上流までの距離がそのまま相手側の下駄になります。

引数の組は全て両環境で通ることが前提です。データを増減したときは `--parity` を流し、
取得に失敗する組が無いか確認してください。
